...
use GOLD_io, only : file_exists, read_data, slasher
...
!                 call to MESO_surface_forcing_init
  ...
  call GOLD_error(FATAL, "MESO_wind_surface_forcing: " // &
...
!                 call to MESO_surface_forcing_init
  ...
  if (CS%restorebuoy) then
    if (CS%use_temperature) then
      ...
      !   When modifying the code, comment out this error message.  It is here
      ! so that the original (unmodified) version is not accidentally used.
!      call GOLD_error(FATAL, "MESO_buoyancy_surface_forcing: " // &
...
!                 for this module
  ...
  if (associated(CS)) then
    call GOLD_error(WARNING, "MESO_surface_forcing_init called with an associated "// &
                             "control structure.")
    return
  endif
  allocate(CS)
