...
use GOLD_io, only : file_exists, read_data, slasher, vardesc, num_timelevels
...
!           time_lev-1,CS%wind_last_lev-1
    ...
    case ("C")
      if (G%symmetric) then
        if (.not.associated(G%Domain_aux)) call GOLD_error(FATAL, &
...
!                              flow control module.
  ...
  if (trim(CS%buoy_config) == "file") then
    call get_param(param_file, mod, "ARCHAIC_OMIP_FORCING_FILE", CS%archaic_OMIP_file, &
                 ...
                 "false, use the variable groupings available from GOLD \n"//&
